...
self = Self(
    patch_embed=video_model.patch_embed, num_target_blocks=video_model.num_target_blocks
)
...
def patch_idx_to_coords_3d(
    idx: int, patch_size: Tuple[int, int, int], video_size: Tuple[int, int, int]
) -> Tuple[int, int, int]:
    """
    Args:
        idx (int): The patch index.
        patch_size (Tuple[int, int, int]): Size of the patch in (temporal, height, width).
        video_size (Tuple[int, int, int]): Size of the video in (temporal, height, width).

    Returns:
        Tuple[int, int, int]: The (temporal, height, width) pixel coordinates of the top-left corner of the patch.
    """
    # Number of patches along each dimension (temporal, height, width)
    patches_per_depth: int = video_size[0] // patch_size[0]  # Temporal patches
    patches_per_row: int = video_size[1] // patch_size[1]  # Height patches
    patches_per_col: int = video_size[2] // patch_size[2]  # Width patches

    # Calculate the position in the 3D grid of patches
    depth = idx // (patches_per_row * patches_per_col)
    row = (idx % (patches_per_row * patches_per_col)) // patches_per_col
    col = idx % patches_per_col

    # Convert patch grid position to pixel coordinates
    temporal_coord = depth * patch_size[0]
    height_coord = row * patch_size[1]
    width_coord = col * patch_size[2]

    return temporal_coord, height_coord, width_coord


def visualise_patches_3d(self: Self, x: torch.Tensor):
    ...
    print(f"{x.shape=}")  # torch.Size([75, 3, 224, 224])

    num_patches_per_image: int = sum(self.patch_embed.patch_shape)

    for image_idx, image_tensor in enumerate(x):
        unnormalised_image: np.array = unnormalise_image(image_tensor=image_tensor)
        ...
        for context_patch in context_patches:
            top_left = patch_idx_to_coords_2d(
                context_patch - num_patches_per_image * image_idx,
                patch_size,
                image_size,
            )
            bottom_right = (top_left[0] + patch_size[0], top_left[1] + patch_size[1])
            draw.rectangle(
                [top_left, bottom_right], outline="black", width=2, fill=None
            )

        # Draw the target patches in red
        for target_patch in all_unique_target_patches:
            top_left = patch_idx_to_coords_2d(
                target_patch - num_patches_per_image * image_idx, patch_size, image_size
            )
            bottom_right = (top_left[0] + patch_size[0], top_left[1] + patch_size[1])
            draw.rectangle(
                [top_left, bottom_right], outline="green", width=2, fill=None
            )
...
visualise_patches_3d(self=self, x=image)
