...
class IJEPA(JEPA_base, pl.LightningModule):
    ...
    def forward(
        ...
        target_patches, all_unique_target_patches = (
            JEPA_base.generate_target_patches_2d(
        ...
        context_patches: List[int] = JEPA_base.generate_context_patches_2d(
            # patch_dim=self.patch_dim,
            patch_dim=self.patch_embed.patch_shape,
...
class VJEPA(JEPA_base, pl.LightningModule):
    ...
    def forward(
        ...
        batch_size, time, channels, img_height, img_width = x.shape

        target_patches: List[List[int]]
        all_unique_target_patches: List[int]
        target_patches, all_unique_target_patches = (
            JEPA_base.generate_target_patches_3d(
                # patch_dim=self.patch_dim,
                patch_dim=self.patch_embed.patch_shape,
                aspect_ratio=target_aspect_ratio,
                scale=target_scale,
        ...
        context_patches: List[int] = JEPA_base.generate_context_patches_3d(
            # patch_dim=self.patch_dim,
            patch_dim=self.patch_embed.patch_shape,
            aspect_ratio=context_aspect_ratio,
            scale=context_scale,
        ...
        return self.forward_base(
            x=x,  # (batch_size, time, channels, img_height, img_width)
            target_patches=target_patches,
            context_patches=context_patches,
        )
