...
class IJEPA_FT(pl.LightningModule):
    ...
    def forward(self, x, random_t):
        ...
        x = self.pretrained_model.predictor.predictor_embed(x)
        x = self.pretrained_model.predictor.decoder(x)
        x = self.pretrained_model.predictor.predictor_proj(
            self.pretrained_model.predictor.predictor_norm(x)
        )
        #########################
    ...
    def training_step(self, batch, batch_idx):
        ...
        for clip in batch:
            ...
            save_frames_to_folder(
                video_tensor=stacked_img,
                original_tensor=y,
                folder_name="finetune2/images/stacked",
                batch_idx=batch_idx,
            )
    ...
    def mask_image(self, image_tensor):
        # Define mask parameters
        top, left = 50, 50  # Top-left corner of the mask
        mask_height, mask_width = 80, 80  # Size of the mask

        # Apply the mask (set values to 0)
        image_tensor[:, :, :, top : top + mask_height, left : left + mask_width] = 0
